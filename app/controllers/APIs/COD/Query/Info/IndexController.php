...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->cod->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                if(in_array($response['gamename'], array('Call of Duty','main')) || !empty($response['codextended'])) {
                    ...
                    $output['name']                 = $response['sv_hostname'];
                    $output['map']                  = $response['mapname'];
                    $output['secured']              = ($response['sv_pure'] == 1) ? true : false;
                    $output['password_protected']   = ($response['pswrd'] == 1) ? true : false;
                    $output['version']              = (!empty($response['codextended'])) ? "CoDExtended ".$response['shortversion'] : $response['shortversion'];
                    ...
                    $output['queryPort'] = $response['gq_port_query'];
                    $output['protocol']  = $response['gq_transport'];
                    $output['error']     = "Server is not running Call of Duty. (".$response['gamename'].")";
                    $output['code']      = 004;
                }
            }
            $output['cached'] = true;
    ...
    public function multiAction() {
        ...
        foreach ($cConfig['addresses'] as $key => $value) {
            ...
            $combinedRedis = $this->config->application->redis->keyStructure->cod->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['online']) {
                    ...
                    if(in_array($response['gamename'], array('Call of Duty','main')) || !empty($response['codextended'])) {
                        ...
                        $output[$combined]['name']                  = $response['sv_hostname'];
                        $output[$combined]['map']                   = $response['mapname'];
                        $output[$combined]['secured']               = ($response['sv_pure'] == 1) ? true : false;
                        $output[$combined]['password_protected']    = ($response['pswrd'] == 1) ? true : false;
                        $output[$combined]['version']               = (!empty($response['codextended'])) ? "CoDExtended ".$response['shortversion'] : $response['shortversion'];
                        ...
                        $output[$combined]['queryPort'] = $response['gq_port_query'];
                        $output[$combined]['protocol']  = $response['gq_transport'];
                        $output[$combined]['error']     = "Server is not running Call of Duty. (".$response['gamename'].")";
