...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->coduo->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                if(in_array($response['gamename'], array('CoD:United Offensive'))) {
                    ...
                    $output['version']              = $response['shortversion'];
                    ...
                    $output['queryPort'] = $response['gq_port_query'];
                    $output['protocol']  = $response['gq_transport'];
                    $output['error']     = "Server is not running Call of Duty: United Offensive. (".$response['gamename'].")";
                    $output['code']      = 004;
                }
            }
            $output['cached'] = true;
    ...
    public function multiAction() {
        ...
        foreach ($cConfig['addresses'] as $key => $value) {
            ...
            $combinedRedis = $this->config->application->redis->keyStructure->coduo->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['online']) {
                    ...
                    if(in_array($response['gamename'], array('CoD:United Offensive'))) {
                        ...
                        $output[$combined]['version']               = $response['shortversion'];
                        ...
                        $output[$combined]['queryPort'] = $response['gq_port_query'];
                        $output[$combined]['protocol']  = $response['gq_transport'];
                        $output[$combined]['error']     = "Server is not running Call of Duty: United Offensive. (".$response['gamename'].")";
