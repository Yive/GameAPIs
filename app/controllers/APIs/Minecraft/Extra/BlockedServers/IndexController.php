...
class IndexController extends ControllerBase {
    ...
    public function indexAction() {
        ...
        $cConfig['redis']['key']['listExtended'] = $this->config->application->redis->keyStructure->mcpc->blockedservers->listExtended;
        $redis = new Redis();
        $redis->pconnect($cConfig['redis']['host']);
        if($redis->exists($cConfig['redis']['key']['listExtended'])) {
            $list = json_decode($redis->get($cConfig['redis']['key']['listExtended']),true);
            ...
            if (!$redis->exists($cConfig['redis']['key']['list'])) {
                ...
                $redis->set($cConfig['redis']['key']['list'], json_encode($json['blocked']), 60);
            } else {
                $json['blocked'] = json_decode($redis->get($cConfig['redis']['key']['list']),true);
            }
            foreach ($json['blocked'] as $value) {
                if(empty($value)) {
                    continue;
                }
                if(!$redis->exists($cConfig['redis']['key']['check'].$value)) {
                    $json['unknown'][$value]['sha1'] = $value;
                } else {
                    $check = json_decode($redis->get($cConfig['redis']['key']['check'].$value),true);
                    if($check['domain'] == NULL) {
                        $redis->del($cConfig['redis']['key']['check'].$value);
                        continue;
                    }
    ...
    public function checkAction() {
        ...
        $noipDomains = array("ddns.net","ddnsking.com","3utilities.com","bounceme.net","freedynamicdns.net","freedynamicdns.org","gotdns.ch","hopto.org","myftp.biz","myftp.org","myvnc.com","onthewifi.com","redirectme.net","servebeer.com","serveblog.net","servecounterstrike.com","serveftp.com","servegame.com","servehalflife.com","servehttp.com","serveirc.com","serveminecraft.net","servemp3.com","servepics.com","servequake.com","sytes.net","viewdns.net","webhop.me","zapto.org","access.ly","blogsyte.com","brasilia.me","cable-modem.org","ciscofreak.com","collegefan.org","couchpotatofries.org","damnserver.com","ddns.me","ditchyourip.com","dnsfor.me","dnsiskinky.com","dvrcam.info","dynns.com","eating-organic.net","fantasyleague.cc","geekgalaxy.com","golffan.us","health-carereform.com","homesecuritymac.com","homesecuritypc.com","hosthampster.com","hopto.me","ilovecollege.info","loginto.me","mlbfan.org","mmafan.biz","myactivedirectory.com","mydissent.net","myeffect.net","mymediapc.net","mypsx.net","mysecuritycamera.com","mysecuritycamera.net","mysecuritycamera.org","net-freaks.com","nflfan.org","nhlfan.net","pgafan.net","point2this.com","pointto.us","privatizehealthinsurance.net","quicksytes.com","read-books.org","securitytactics.com","serveexchange.com","servehumour.com","servep2p.com","servesarcasm.com","stufftoread.com","ufcfan.org","unusualperson.com","workisboring.com","allowed.org");
        if (!$redis->exists($cConfig['redis']['key']['list'])) {
            $blockedservers = array_filter(explode(PHP_EOL, file_get_contents('https://sessionserver.mojang.com/blockedservers')));
            $redis->set($cConfig['redis']['key']['list'], json_encode($blockedservers), 60);
        } else {
            $blockedservers = json_decode($redis->get($cConfig['redis']['key']['list']),true);
        }
        foreach ($ips as $ip) {
            $i=0;
            if(in_array(extract_domain($ip), $noipDomains)) {
                ...
                if (!$redis->exists($cConfig['redis']['key']['check'].$sha1)) {
                    $redis->set($cConfig['redis']['key']['check'].$sha1, json_encode($redisSet, JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE));
                }
