...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        if(strpos($params['ip'], ':')) {
            ...
            $params['port'] = $explodeParams[1] ?? 28015;
        } else {
            $params['port'] = 28015;
        }
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->rust->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                if(in_array($response['game_dir'], array('rust'))) {
                    ...
                    $output['queryPort'] = $response['gq_port_query'];
                    $output['protocol']  = $response['gq_transport'];
                    $output['error']     = "Server is not running Rust. (".$response['game_descr']." - ".$response['game_id'].")";
                    $output['code']      = 004;
                    if($cConfig['debug']) {
                        $output['debug'] = $response;
                    }
    ...
    public function multiAction() {
        ...
        foreach ($explodeComma as $key => $value) {
            if(strpos($value, ':')) {
                ...
                $cConfig['addresses'][$i]['port'] = 28015;
            }
            $i++;
        }
        foreach ($cConfig['addresses'] as $key => $value) {
            $combined = $value['ip'].':'.$value['port'];
            $combinedRedis = $this->config->application->redis->keyStructure->rust->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['online']) {
                    ...
                    if(in_array($response['game_dir'], array('rust'))) {
                        ...
                        $output[$combined]['queryPort'] = $response['gq_port_query'];
                        $output[$combined]['protocol']  = $response['gq_transport'];
                        $output[$combined]['error']     = "Server is not running Rust. (".$response['game_descr']." - ".$response['game_id'].")";
