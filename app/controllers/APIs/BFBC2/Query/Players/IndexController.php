...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        if(strpos($params['ip'], ':')) {
            ...
            $params['port'] = $explodeParams[1] ?? 19567;
        } else {
            $params['port'] = 19567;
        }
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->bfbc2->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                $output['queryPort'] = $response['gq_port_query']   ?? $cConfig['port'] + 29321;
    ...
    public function multiAction() {
        ...
        foreach ($explodeComma as $key => $value) {
            if(strpos($value, ':')) {
                ...
                $cConfig['addresses'][$i]['port'] = (int) $explodeParams[1] ?? 19567;
            } else {
                $cConfig['addresses'][$i]['ip'] = $value;
                $cConfig['addresses'][$i]['port'] = 19567;
            }
            $i++;
        }
        foreach ($cConfig['addresses'] as $key => $value) {
            $combined = $value['ip'].':'.$value['port'];
            $combinedRedis = $this->config->application->redis->keyStructure->bfbc2->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['gq_online']) {
                    ...
                    $output[$combined]['queryPort'] = $response['gq_port_query']    ?? $value['port'] + 29321;
