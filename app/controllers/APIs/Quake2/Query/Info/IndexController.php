...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        if(strpos($params['ip'], ':')) {
            ...
            $params['port'] = $explodeParams[1] ?? 27910;
        } else {
            $params['port'] = 27910;
        }
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->quake2->ping.$cConfig['ip'].':'.$cConfig['port'];
    ...
    public function multiAction() {
        ...
        foreach ($explodeComma as $key => $value) {
            if(strpos($value, ':')) {
                ...
                $cConfig['addresses'][$i]['port'] = 27910;
            }
            $i++;
        }
        foreach ($cConfig['addresses'] as $key => $value) {
            $combined = $value['ip'].':'.$value['port'];
            $combinedRedis = $this->config->application->redis->keyStructure->quake2->ping.$combined;
