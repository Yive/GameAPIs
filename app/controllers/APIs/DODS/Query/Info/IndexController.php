...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->dods->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                if(in_array($response['game_descr'], array('Day of Defeat: Source','Day of Defeat'))) {
                    ...
                    $output['queryPort'] = $response['gq_port_query'];
                    $output['protocol']  = $response['gq_transport'];
                    $output['error']    = "Server is not running Day of Defeat: Source. (".$response['game_descr']." - ".$response['game_id'].")";
                    $output['code']      = 004;
                }
            }
            $output['cached'] = true;
    ...
    public function multiAction() {
        ...
        foreach ($cConfig['addresses'] as $key => $value) {
            ...
            $combinedRedis = $this->config->application->redis->keyStructure->dods->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['online']) {
                    ...
                    if(in_array($response['game_descr'], array('Day of Defeat: Source','Day of Defeat'))) {
                        ...
                        $output[$combined]['protocol']  = $response['gq_transport'];
                        $output[$combined]['error']    = "Server is not running Day of Defeat: Source. (".$response['game_descr']." - ".$response['game_id'].")";
