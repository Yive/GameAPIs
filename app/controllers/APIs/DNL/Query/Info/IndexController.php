...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->dnl->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                if(in_array($response['game_dir'], array('Dark and light'))) {
                    ...
                    $output['version']              = (!empty($response['BuildId_s']) && !empty($response['MINORBUILDID_s'])) ? 'v'.$response['BuildId_s'].'.'.$response['MINORBUILDID_s'] : $response['version'];
    ...
    public function multiAction() {
        ...
        foreach ($cConfig['addresses'] as $key => $value) {
            ...
            $combinedRedis = $this->config->application->redis->keyStructure->dnl->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['online']) {
                    ...
                    if(in_array($response['game_dir'], array('Dark and light'))) {
                        ...
                        $output[$combined]['version']               = (!empty($response['BuildId_s']) && !empty($response['MINORBUILDID_s'])) ? 'v'.$response['BuildId_s'].'.'.$response['MINORBUILDID_s'] : $response['version'];
