        ...
        if($redis->exists($this->config->application->redis->keyStructure->css->ping.$params['ip'])) {
            ...
            if(!$response['gq_online']) {
                ...
                if(in_array($response['game_descr'], array('Counter-Strike: Source'))) {
                    ...
                    $output['hostname'] = $response['gq_address'];
                    $output['port']     = $response['gq_port_client'];
                    $output['error']    = "Server is not running Counter-Strike: Source. It's running ".$response['game_descr'];
                }
            }
            $output['cached'] = true;
        } else {
            ...
            if($redis->exists($this->config->application->redis->keyStructure->css->ping.$combined)) {
                ...
                if(!$response['online']) {
                    ...
                    if(in_array($response['game_descr'], array('Counter-Strike: Source'))) {
                        ...
                        $output[$combined]['hostname'] = $response['gq_address'];
                        $output[$combined]['port']     = $response['gq_port_client'];
                        $output[$combined]['error']    = "Server is not running Counter-Strike: Source. It's running ".$response['game_descr'];
                    }
                }
