...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->cmw->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                $output['queryPort'] = $response['gq_port_query']   ?? $cConfig['port'] + 19238;
                $output['protocol']  = $response['gq_transport']    ?? "udp";
                $output['error']     = "Couldn't connect to address.";
                $output['code']      = 003;
            } else {
                if(in_array($response['game_dir'], array('chivalrymedievalwarfare'))) {
                    ...
                    $output['queryPort'] = $response['gq_port_query'];
                    $output['protocol']  = $response['gq_transport'];
                    $output['error']     = "Server is not running Chivalry: Medieval Warfare. (".$response['game_descr']." - ".$response['game_id'].")";
                    $output['code']      = 004;
                }
            }
            $output['cached'] = true;
            ...
            unset($response['keywords']); // Seems to just include a broken packet which json doesn't like.
    ...
    public function multiAction() {
        ...
        foreach ($explodeComma as $key => $value) {
            if(strpos($value, ':')) {
                ...
                $cConfig['addresses'][$i]['port'] = (int) $explodeParams[1] ?? 7777;
            } else {
        ...
        foreach ($cConfig['addresses'] as $key => $value) {
            ...
            $combinedRedis = $this->config->application->redis->keyStructure->cmw->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['gq_online']) {
                    ...
                    $output[$combined]['queryPort'] = $response['gq_port_query']    ?? $value['port'] + 19238;
                	...
                	if(in_array($response['game_dir'], array('chivalrymedievalwarfare'))) {
                        ...
                        $output[$combined]['queryPort'] = $response['gq_port_query'];
                        $output[$combined]['protocol']  = $response['gq_transport'];
                        $output[$combined]['error']     = "Server is not running Chivalry: Medieval Warfare. (".$response['game_descr']." - ".$response['game_id'].")";
