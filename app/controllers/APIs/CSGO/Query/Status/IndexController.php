...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        $cConfig['redis']['key']  = $this->config->application->redis->keyStructure->csgo->ping.$cConfig['ip'].':'.$cConfig['port'];
        ...
        if($redis->exists($cConfig['redis']['key'])) {
            ...
            if(!$response['gq_online']) {
                ...
                if(in_array($response['game_dir'], array('csgo'))) {
                    ...
                    $output['queryPort'] = $response['gq_port_query'];
                    $output['protocol']  = $response['gq_transport'];
                    $output['error']     = "Server is not running Counter-Strike: Global Offensive. (".$response['game_descr']." - ".$response['game_id'].")";
                    $output['code']      = 004;
                    if($cConfig['debug']) {
                        $output['debug'] = $response;
                    }
    ...
    public function multiAction() {
        ...
        foreach ($cConfig['addresses'] as $key => $value) {
            ...
            $combinedRedis = $this->config->application->redis->keyStructure->csgo->ping.$combined;
            if($redis->exists($combinedRedis)) {
                ...
                if(!$response['online']) {
                    ...
                    if(in_array($response['game_dir'], array('csgo'))) {
                        ...
                        $output[$combined]['queryPort'] = $response['gq_port_query'];
                        $output[$combined]['protocol']  = $response['gq_transport'];
                        $output[$combined]['error']     = "Server is not running Counter-Strike: Global Offensive. (".$response['game_descr']." - ".$response['game_id'].")";
