...
class IndexController extends ControllerBase {
    ...
    public function singleAction() {
        ...
        if(!strpos($params['ip'], ':')) {
            $params['ip'] = $params['ip'].':2302';
        }
        if($redis->exists($this->config->application->redis->keyStructure->arma3->ping.$params['ip'])) {
            $response = json_decode(base64_decode($redis->get($this->config->application->redis->keyStructure->arma3->ping.$params['ip'])),true);
            if(!$response['gq_online']) {
                ...
                if(in_array($response['game_dir'], array('Arma3'))) {
                    ...
                    $output['hostname'] = $response['gq_address'];
                    $output['port']     = $response['gq_port_client'];
                    $output['error']    = "Server is not running ARMA 3. It's running ".$response['game_descr']." (".$response['game_id'].")";
                }
            }
            $output['cached'] = true;
        } else {
            $GameQ = new \GameQ\GameQ();
            $GameQ->addServer(['type' => 'arma3','host'=> $params['ip']]);
            ...
            $output['cached'] = false;
            $redis->set($this->config->application->redis->keyStructure->arma3->ping.$params['ip'], base64_encode(json_encode($response, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)), 15);
        }
    ...
    public function multiAction() {
        ...
        foreach ($explodeComma as $key => $value) {
            if(strpos($value, ':')) {
                ...
                $params['addresses'][$i]['port'] = 2302;
            }
            $i++;
        }
        foreach ($params['addresses'] as $key => $value) {
            $combined = $value['ip'].':'.$value['port'];
            if($redis->exists($this->config->application->redis->keyStructure->arma3->ping.$combined)) {
                $response = json_decode(base64_decode($redis->get($this->config->application->redis->keyStructure->arma3->ping.$combined)),true);
                if(!$response['online']) {
                	...
                	if(in_array($response['game_dir'], array('Arma3'))) {
                        ...
                        $output[$combined]['hostname'] = $response['gq_address'];
                        $output[$combined]['port']     = $response['gq_port_client'];
                        $output[$combined]['error']    = "Server is not running ARMA 3. It's running ".$response['game_descr']." (".$response['game_id'].")";
                    }
                }
                $output[$combined]['cached'] = true;
            } else {
                $GameQ = new \GameQ\GameQ();
                $GameQ->addServer(['type' => 'arma3','host'=> $combined]);
                $GameQ->setOption('timeout', 2); // seconds
                ...
                $redis->set($this->config->application->redis->keyStructure->arma3->ping.$combined, base64_encode(json_encode($response, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)), 15);
            }
        }
        echo json_encode($output, JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
    }
