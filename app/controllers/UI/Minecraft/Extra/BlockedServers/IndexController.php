...
class IndexController extends ControllerBase {
    ...
    public function indexAction() {
        ...
        $blockedserversList = array();
        foreach ($blockedservers as $key => $value) {
            if($redis->exists($this->config->application->redis->keyStructure->mcpc->blockedservers->check.$value)) {
                $details = json_decode($redis->get($this->config->application->redis->keyStructure->mcpc->blockedservers->check.$value),true);
                array_push($blockedserversList, array("sha1" => $details['sha1'], "ip" => $details['domain']));
            } else {
    ...
    public function checkAction() {
        ...
        foreach ($output as $domain => $domainValue) {
            echo '<div class="column is-12"><h1 class="title">'.$domain.'</h1></div>';
            foreach ($domainValue as $result => $resultValue) {
                if($resultValue['blocked']) {
                    echo '<div class="column is-half">';
                    echo '<article class="message is-danger">';

                    echo '<div class="message-header">';
                    echo '<p>'.$resultValue['domain'].'</p>';
                    echo '</div>';

                    echo '<div class="message-body">';

                    echo '<p>';
                    echo '<strong>Domain:</strong> '.$resultValue['domain'].'<br/>';
                    echo '<strong>SHA1:</strong> '.$resultValue['sha1'].'<br/>';
                    ...
                    echo '<strong>Domain:</strong> '.$resultValue['domain'].'<br/>';
                    echo '<strong>SHA1:</strong> '.$resultValue['sha1'].'<br/>';
                    echo '<strong>Status:</strong> Not Blocked<br/>';
                    echo '</p>';
