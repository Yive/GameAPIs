...
class MCPing
{
	...
	public function GetStatus($Hostname = '127.0.0.1', $Port = 25565, $IsOld17 = false, $Timeout = 1) {
		...
		if (!is_bool($IsOld17)) {
			$this->error = 'Invalid parameter for oldProtocolServers';
			return $this;
		}
	...
	private function Ping() {
		...
		do {
			...
			if (!$block) {
				$this->error = 'Server returned too little data.';
				return $this;
			}
			$Data .= $block;
		...
		if (json_last_error() !== JSON_ERROR_NONE) {
			...
			$this->error = 'Internal JSON error.';
			return $this;
		}
		if($Data == "Server is still starting! Please wait before reconnecting.") {
			$this->error = 'Server is still starting up.';
			return $this;
		}
